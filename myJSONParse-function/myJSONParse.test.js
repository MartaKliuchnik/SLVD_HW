...
describe('Parsing JSON strings with myJSONParse: ', () => {
	...
	test('parse a nested object correctly', () => {
		...
		const jsonString =
			'{"name": "John", "age": 30, "phone": false, "adress": {"town": "New Yourk"}}';
		const expectedObject = {
			name: 'John',
			age: 30,
			phone: false,
			adress: { town: 'New Yourk' },
		};
	...
	test('should throw an error if unquoted keys are found', () => {
		...
		expect(() => myJSONParse(jsonString)).toThrow(
			'Missing quotes around key "name" at position 2'
		);
	});
	...
	test('should throw an error if parse an invalid array', () => {
		...
		expect(() => myJSONParse(jsonString)).toThrow(
			'Unexpected token in array - expected "]"'
		);
	});
});
