...
const datePattern = require('../utils/datePattern.js');
...
class Tokenization {
	...
	checkUnquotedKeys() {
		let unquotedMatch;
		if (!datePattern.test(this.jsonString)) {
			while (
				(unquotedMatch = unquotedKeyPattern.exec(this.jsonString)) !== null
			) {
				const unquotedKey = unquotedMatch[1];
				const index = unquotedMatch.index;
				if (!datePattern.test(unquotedKey)) {
					ErrorHandling.throwError(
						`Missing quotes around key "${unquotedKey}"`,
						index
					);
				}
			}
	...
	checkUnquotedStringMatch() {
		...
		while (
			(unquotedStringMatch = invalidUnquotedStringPattern.exec(
				this.jsonString
			)) !== null
		) {
			const invalidUnquotedString = unquotedStringMatch[1];
			const index =
				unquotedStringMatch.index +
				unquotedStringMatch[0].indexOf(invalidUnquotedString);
